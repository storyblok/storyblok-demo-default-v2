...
<template>
  <Transition name="slide">
    ...
    >
      <h2 class="mb-4 font-display text-2xl font-black uppercase text-white">
        Navigation
      </h2>
      <ul v-if="!autoNav" class="flex flex-col space-y-4 text-lg">
        <li v-for="item in headerNav" :key="item._uid">
          <NavItem class="text-white" :item="item" />
        </li>
      </ul>
      <ul v-else class="flex flex-col space-y-4 text-lg">
